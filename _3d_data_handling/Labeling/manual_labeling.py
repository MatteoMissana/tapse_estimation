...
def reannotate_points_in_h5(file_path, save_path, num_landmarks=3, LUT=None):
    ...
    with h5py.File(file_path, 'r') as h5_file:
        ...
        if 'annotations' in h5_file:
            ...
            ref_coord = np.zeros(ref_coord.shape)
    ...
    while True:
        ...
        if idx in rejected_frames:
            h, w = frames.shape[:2]
            plt.text(w//2, h//2, 'X', color='red', fontsize=60, ha='center', va='center', alpha=0.6)

        user_input = None
        plt.gcf().canvas.mpl_connect('key_press_event', press)

        print(f"Frame {idx + 1}/{frames.shape[2]}")
        print("Enter = select landmark, C = close, Arrow keys = navigate frames, R = reject frame")
        ...
        if user_input == "enter":
            ...
            if idx in rejected_frames:
                rejected_frames.remove(idx)
                print(f"Frame {idx} unmarked for rejection.")
            else:
                rejected_frames.add(idx)
                print(f"Frame {idx} marked for rejection.")
    ...
    with h5py.File(file_path, 'r') as h5_file, h5py.File(save_path, 'w') as new_h5_file:
        for key in h5_file.keys():
            if key == 'frames':
                kept_indices = [i for i in range(frames.shape[2]) if i not in rejected_frames]
                new_h5_file.create_dataset('frames', data=frames[:, :, kept_indices])
            elif key != 'annotations':
                h5_file.copy(key, new_h5_file)

        kept_ref_coord = np.delete(ref_coord, list(rejected_frames), axis=0)
        new_h5_file.create_dataset('annotations', data=kept_ref_coord)
...
if __name__ == "__main__":
    ...
    reannotate_points_in_h5(r'd:\mmissana\data\RV_PATIENTS\RV_patients_annotated_renamed\116\P429QJHU.h5', r'd:\mmissana\data\RV_PATIENTS\RV_patients_annotated_renamed\116\P429QJHU_corrected.h5', num_landmarks=3, LUT=LUT)