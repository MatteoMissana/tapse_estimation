...
- [`training.train_model`](2d/training.py) — training pipeline
- [`indices_prediction.predict_indices`](2d/indices_prediction.py) — inference + index calculation pipeline
- [`statystical_analysis.analysis`](2d/statystical_analysis.py) — statistical / Bland–Altman analysis
- [`test_models.process_h5_file_single`](2d/test_models.py) — per-h5 testing & diagnostics
...
- Models & init: [2d/models/models.py](2d/models/models.py), [2d/models/improved_unet.py](2d/models/improved_unet.py), [2d/models/weights_initialization.py](2d/models/weights_initialization.py)
...
- Saved output / experiments: [2d/results/](2d/results/) and [2d/runs/](2d/runs/)
...
Key endpoints and symbols:
- [`training.train_model`](2d/training.py)
- [`training.validate`](2d/training.py)
- [`training.Tester`](2d/training.py)

What it does:
- Loads datasets using `KeypointDataset` ([2d/dataloader/main.py](2d/dataloader/main.py)).
- Builds model (many options supported: monai U-Net, ResNet regressors, improved U-Net, etc.) from [2d/models/models.py](2d/models/models.py).
- Trains using OrderedDistanceLoss or GaussianKeypointLoss ([2d/losses/distances.py](2d/losses/distances.py)).
...
python 2d/training.py \
  --train_data "D:\mmissana\data\RV_PATIENTS\dataset_after_review\train.npz" \
...
See: [`training.train_model`](2d/training.py) and [2d/models/models.py](2d/models/models.py).

---

### 2) Inference & indices: [2d/indices_prediction.py](2d/indices_prediction.py)
Key symbol:
- [`indices_prediction.predict_indices`](2d/indices_prediction.py)
...
- --tapse {distance,projection}
- --reduction {mean,max,min}
- --save_images (save visualizations)
- --threshold (center-of-mass threshold)
- --two_dimensional (flag for reorientation)
- --best_combination (use heuristic combination of min/max/mean across indices)

Example:
python 2d/indices_prediction.py \
  --h5_dir "D:\mmissana\data\RV_PATIENTS\RV_patients_converted" \
  --excel_path "2d/results/some_list.xlsx" \
  --model_path "2d/runs/best_unet/best_model.pth" \
  --depth 6 --filters 16 --residuals 0 \
  --filter --tapse distance --reduction max --save_images --threshold 0.875

See: [`indices_prediction.predict_indices`](2d/indices_prediction.py), [`postprocessing.indices_calculation`](2d/postprocessing/indices_calculation.py), [`postprocessing.kalman_filter.KalmanFilter`](2d/postprocessing/kalman_filter.py).

---

### 3) Statistical analysis: [2d/statystical_analysis.py](2d/statystical_analysis.py)
Key symbol:
- [`statystical_analysis.analysis`](2d/statystical_analysis.py)
...
- If `best_combination` or `best_rvlsffw` names are present in automatic Excel, script derives additional indices (RVFAC, RVLSF variants) before analysis.

See: [`statystical_analysis.analysis`](2d/statystical_analysis.py).
...
- HDF5 structure must follow what the scripts expect:
